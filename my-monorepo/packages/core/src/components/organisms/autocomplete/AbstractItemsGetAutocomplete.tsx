import { useAbstractItemsAbstractItemsGetQuery } from "@core/hooks/api/abstract-items/useAbstractItemsQuery";
...
export type AbstractItemsGetAutocompleteProps<
  T = AbstractItemsGet200ResponseRowsInner,
> = {
...
export function AbstractItemsGetAutocomplete<
  T = AbstractItemsGet200ResponseRowsInner,
>({
  onSelect,
...
}: AbstractItemsGetAutocompleteProps<T>) {
  ...
  } as DefaultApiAbstractItemsGetRequest);
  ...
  useEffect(() => {
    const rows: T[] = ((
      res as unknown as {
        data: AbstractItemsGet200Response;
      }
    )?.data.rows ?? []) as T[];
    if (!rows) return;
