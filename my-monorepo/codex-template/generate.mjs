...
function genHooksModule({ resource, queries }) {
  ...
  const hookBlocks = queries
    .map((q) => {
      const hasParams = !!q.requestType;
      const hookName = `use${resourcePascal}${pascalCase(q.name)}Query`;
      const defaultOptions =
        q.name.toLowerCase().includes("select") && hasParams
          ? `staleTime: 1000 * 60 * 5,\n    enabled: !!params,`
          : `staleTime: 1000 * 60,\n    retry: 1,`;
      const sig = hasParams ? `(params: ${q.requestType})` : `()`;
...
function genMutationHooksModule({ resource, mutations }) {
  const resourcePascal = pascalCase(resource);
  const resourceVar = camelCase(resource);
  const hookImports = unique(
    mutations.map((m) => m.requestType).filter(Boolean)
  );

  const hookBlocks = mutations
    .map((m) => {
...
function main() {
  ...
  if (configIndex === -1 || !args[configIndex + 1]) {
    console.error(
      "Usage: node codex-template/generate.mjs --config <path-to-config.json>"
    );
    process.exit(1);
  }
  ...
  const apiOut = path.resolve(
    cwd,
    `packages/core/src/api/${resource}/${resource}Query.ts`
  );
  const hooksOut = path.resolve(
    cwd,
    `packages/core/src/hooks/${resource}/use${pascalCase(resource)}Query.ts`
  );
  const hooksMutOut = path.resolve(
    cwd,
    `packages/core/src/hooks/${resource}/use${pascalCase(resource)}Mutation.ts`
  );
